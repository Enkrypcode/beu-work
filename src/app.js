...
function profileControl(profile) {
  ...
  return `<div class="profile-menu"><button class="profile-button" data-profile aria-label="View account" aria-expanded="${profileOpen}"><span class="profile-avatar">${escape(initials(profile || {full_name:email}))}</span><span class="profile-summary"><strong>${escape(name)}</strong><small>${escape(title)}</small></span></button>${profileOpen ? `<section class="profile-popover" role="dialog" aria-label="Account"><div><p class="eyebrow">ACCOUNT</p><strong>${escape(name)}</strong><small>${escape(title)}</small>${email ? `<small>${escape(email)}</small>` : ''}</div><button class="icon-button" data-close-profile aria-label="Close account">×</button></section>` : ''}</div>`;
}
