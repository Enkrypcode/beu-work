...
let authMessage = '', signingIn = false, authStarting = true;
...
function personLabel(p, clickable = true) {
  return `<${clickable ? 'button' : 'div'} class="person-label" ${clickable ? `data-person="${escape(p?.user_id || '')}"` : ''}><span class="avatar">${escape(initials(p))}</span><span><strong>${escape(personName(p))}</strong><small>${escape(p?.job_title || 'Job title not set')}</small></span></${clickable ? 'button' : 'div'}>`;
}
...
function render() {
  ...
  root.innerHTML = `<div class="shell"><header>${brand()}<nav aria-label="Main navigation">${['dashboard','work','people'].map(n=>`<a href="#${n}" ${(page===n || page==='person'&&n==='people')?'aria-current="page"':''}>${n[0].toUpperCase()+n.slice(1)}</a>`).join('')}</nav><div class="header-actions"><span class="account-name">${escape(personName(own))}<small>${escape(own?.job_title || 'BEU Work account')}</small></span><button class="icon-button" id="theme-toggle" type="button" aria-label="Toggle light or dark mode">${icon('moon')}</button><button class="icon-button" data-signout aria-label="Sign out">${icon('logout')}</button></div></header><main><div class="page-heading"><div><p class="eyebrow">${eyebrow}</p><h1>${title}</h1><p>${copy}</p></div><div class="refresh"><button class="secondary" data-refresh ${refreshing?'disabled':''}>${icon('refresh')}${refreshing?'Refreshing…':'Refresh'}</button><small>${lastRefresh?'Updated '+lastRefresh.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}):'Company-shared work only'}</small></div></div>${error?`<div class="notice error" role="alert"><strong>Shared work is unavailable</strong><p>${escape(error)}</p>${loaded?'<small>The last shared view is hidden until access and sharing are verified again.</small>':''}</div>`:''}${refreshing&&!loaded?'<div class="loading" role="status"><span class="spinner"></span>Loading shared work and profiles…</div>':''}${page==='dashboard'?dashboard():page==='work'?work():page==='people'?peoplePage():personDetailPage()}<footer>${icon('check')} Only intentionally shared work. Personal tasks stay in BEU Priority.</footer></main></div><dialog id="detail"></dialog>`;
  bind();
